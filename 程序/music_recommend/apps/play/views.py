...
from index.models import Song, Dynamic, ShouCang
...
def shoucang(request, music_id):
    song_obj = Song.objects.filter(song_id=music_id).first()
    username = request.session.get('username', '')
    if not username:
        return JsonResponse({"msg": 'ok', 'error': 405})
    user_obj = MyUser.objects.filter(username=username).first()
    ShouCang.objects.create(
        music=song_obj,
        user=user_obj
    )
    return JsonResponse({"msg": 'ok', 'error': 0})
