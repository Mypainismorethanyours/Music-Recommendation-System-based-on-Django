...
from .models import *
# 修改title和header
admin.site.site_title = '基于深度学习的音乐推荐系统后台管理系统'
admin.site.site_header = '基于深度学习的音乐推荐系统'
...
class MusicAdmin(admin.ModelAdmin):
    # 设置模型字段，用于Admin后台数据的表头设置
    list_display = ['id','name','singer','pid','style','url']
    # 设置可搜索的字段并在Admin后台数据生成搜索框，如有外键应使用双下划线连接两个模型的字段
    search_fields = ['name','pid','singer']
    # 设置过滤器，在后台数据的右侧生成导航栏，如有外键应使用双下划线连接两个模型的字段
...
@admin.register(ShouCang)
class ShouCangAdmin(admin.ModelAdmin):
    # 设置模型字段，用于Admin后台数据的表头设置
    list_display = ['id','music','user','create_time']
    # 设置可搜索的字段并在Admin后台数据生成搜索框，如有外键应使用双下划线连接两个模型的字段
